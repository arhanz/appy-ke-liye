...
export default function ViewFlight({
    ...
    const bookFlightHandler = (e) => {
        ...
        bookFlight({
            ...
            .then(res => {
                ...
                enqueueSnackbar('New Booking Created.', {
                    variant: 'success'
                })
                setNewRender('menu')
                setRender('all')
            ...
            .catch(error => {
                if (error.response.status === 401) {
                    ...
                    enqueueSnackbar('Unable to add new booking.', {
                        variant: 'error'
                    })
                }
                dispatch(unsetLoading())
    ...
    return ( <
        ...
        style = {
            ...
            `From: ${data?.from?.label} (${data?.from?.value})`
        } < /p> <
        p className = 'm-0'
        style = {
            {
            ...
            `To: ${data?.to?.label} (${data?.to?.value})`
        } < /p> <
        p className = 'm-0'
        style = {
            {
            ...
            `Type: ${data?.itineraries?.length === 2 ? "Return" : "One Way"}`
        } < /p> {
            checkIndirect(data?.itineraries) && < p className = 'm-0'
            style = {
                    {
                        fontWeight: 'bold'
                    }
            ...
            style = {
                    ...
                    `Passenger: ${parseInt(data?.adult) + parseInt(data?.children) + parseInt(data?.infant)}`
                } < /p> <
                p className = 'm-0'
            style = {
                    {
