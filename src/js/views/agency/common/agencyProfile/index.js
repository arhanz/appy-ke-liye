...
export default function AgencyProfile() {
    ...
    const updateAgencyHandler = (e) => {
        ...
        updateSpecificAgency(agencyId, payload)
            .then(() => {
                enqueueSnackbar("Agency Profile updated successfully.", {
                    variant: "success",
                });
                dispatch(unsetLoading());
            })
            .catch((error) => {
                if (error.response.status === 401) {
                    ...
                    console.log(error.response.data);
                    console.log(error.response.status);
                    enqueueSnackbar("Unable to update agency profile.", {
                        variant: "error",
                    });
                }
                dispatch(unsetLoading());
    ...
    return ( <
        ...
        /CardAvatar> <
        CardBody profile > {
            userRole !== "AGENT" && < DropZone setLogo = {
                setLogo
            }
            />} <
            FormControl className = "mt-4 w-100" >
            <
            InputLabel id = "select-sub-plan-field"
            required >
            ...
            onChange = {
                (e) => setSubscriptionPlan(e.target.value)
            }
            required
            disabled = {
                userRole === "AGENT"
            ...
            onChange = {
                (date) => setSubscriptionActivationDate(date)
            }
            KeyboardButtonProps = {
                {
                    label: "change date",
            ...
            onChange = {
                (date) => setSubscriptionExpiryDate(date)
            }
            KeyboardButtonProps = {
                {
                    label: "change date",
