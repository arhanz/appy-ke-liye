...
import {
    getSpecificTopupRequest,
    approveSpecificTopupRequest,
    rejectSpecificTopupRequest,
} from "crud";
...
export default function EditRequest({
    ...
    const loadValuesHandler = (request) => {
        setRequestee(
            request._requestee ?
            `${request._requestee.firstName} ${request._requestee.lastName}` :
            ""
        );
        setDescription(request.description ? request.description : "");
        setCreatedAt(
            request.createdAt ? Moment(request.createdAt).format("YYYY-MM-DD") : ""
        );
    ...
    const acceptRequestHandler = () => {
        ...
        approveSpecificTopupRequest(id)
            .then(() => {
                enqueueSnackbar("Topup Request Approved.", {
                    variant: "success",
                });
                dispatch(unsetLoading());
                setRender("all");
    ...
    const rejectRequestHandler = () => {
        ...
        rejectSpecificTopupRequest(id)
            .then(() => {
                enqueueSnackbar("Topup Request Rejected.", {
                    variant: "success",
                });
                dispatch(unsetLoading());
                setRender("all");
            ...
            .catch((error) => {
                if (error.response.status === 401) {
                    ...
                    console.log(error.response.data);
                    console.log(error.response.status);
                    enqueueSnackbar("Unable to reject this request.", {
                        variant: "error",
                    });
                }
                dispatch(unsetLoading());
