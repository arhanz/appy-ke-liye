...
export default function EditHotel({
        ...
        getAgencyList(params)
            ...
            .catch((error) => {
                if (error.response.status === 401) {
                    ...
                    console.log(error.response.data);
                    console.log(error.response.status);
                    enqueueSnackbar("Unable to fetch agencies list.", {
                        variant: "error",
                    });
                    setLoadingAgency(false);
                }
    ...
    const updateHotelHandler = (e) => {
        ...
        if (!featuredImg) {
            enqueueSnackbar("Please upload featured image.", {
                variant: "info"
            });
            return;
        }
        ...
        if (phone) payload.contactInfo["phone"] = phone;
        ...
        updateSpecificHotel(id, payload)
            .then((res) => {
                // console.log(res.data)
                enqueueSnackbar("Hotel updated successfully.", {
                    variant: "success"
                });
                dispatch(unsetLoading());
                setRender("all");
            ...
            .catch((error) => {
                if (error.response.status === 401) {
                    ...
                    console.log(error.response.data);
                    console.log(error.response.status);
                    enqueueSnackbar("Unable to update hotel.", {
                        variant: "error"
                    });
                }
                dispatch(unsetLoading());
    ...
    const deleteHandler = () => {
        ...
        deleteHotel(id)
            .then((res) => {
                // console.log(res.data)
                enqueueSnackbar("Hotel delete successfully.", {
                    variant: "success"
                });
                dispatch(unsetLoading());
                setRender("all");
            ...
            .catch((error) => {
                if (error.response.status === 401) {
                    ...
                    console.log(error.response.data);
                    console.log(error.response.status);
                    enqueueSnackbar("Unable to delete hotel.", {
                        variant: "error"
                    });
                }
                dispatch(unsetLoading());
    ...
    const handleFeaturedImageChange = (index) => {
        ...
        enqueueSnackbar("Featured Image Changed.", {
            variant: "info"
        });
    };
    ...
    const handleImageDelete = (index) => {
        ...
        temp.splice(index, 1);
        setImgGallery(temp);
        enqueueSnackbar("Image Delete Success.", {
            variant: "info"
        });
    };
    ...
    return ( <
        ...
        <
        img src = {
            featuredImg ? featuredImg : avatar
        }
        alt = "..."
        style = {
            {
        ...
        div className = "w-100 d-flex justify-content-center flex-wrap my-1" > {
            imgGallery.map((im, i) => {
                return ( <
                    ...
                    className = {
                        `mr-2 mb-2 ${classes.imgGallery}`
                    }
                    alt = "..." /
                    >
                    <
                    div className = {
                        classes.imgGalleryOverlay
                    } >
                    <
        ...
        renderInput = {
            (params) => ( <
                ...
                variant = "standard"
                onChange = {
                    (e) => setSearchAgency(e.target.value)
                }
                label = "Agencies"
                required = {
                    agencies.length === 0
        ...
        onChange = {
            (e, value) => setTags(value)
        }
        className = "w-100 my-4"
        id = "tags"
        required freeSolo options = {
            []
        }
        renderTags = {
            (value, getTagProps) =>
