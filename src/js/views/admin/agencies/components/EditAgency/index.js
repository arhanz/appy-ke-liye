...
import logo_temp from "assets/images/Navbar and footer/FlyWithZOKILogo.webp";
...
export default function EditAgency({ setReload, id, setRender }) {
  ...
  const [subscriptionActivationDate, setSubscriptionActivationDate] = useState(
    new Date()
  );
  ...
  const loadValuesHandler = (agency) => {
    ...
    getSubPlanList()
      .then((res) => {
        ...
        setSubscriptionPlan(
          agency.subscription?.plan ? agency.subscription.plan : ""
        );
      })
      .catch((error) => {
        if (error.response.status === 401) {
          ...
          console.log(error.response.data);
          console.log(error.response.status);
          enqueueSnackbar("Unable to fetch allowed subscription plan list.", {
            variant: "error",
          });
        }
      });
    ...
    setSubscriptionActivationDate(
      agency.subscription?.activationDate
        ? agency.subscription.activationDate
        : null
    );
    setSubscriptionExpiryDate(
      agency.subscription?.expiryDate ? agency.subscription.expiryDate : null
    );
    setAirCommission(
      agency.commission?.air
        ? agency.commission?.air
        : {
            type: "",
            rate: "",
    ...
    setTravelPackageCommission(
      agency.commission?.travelPackage
        ? agency.commission?.travelPackage
        : {
            type: "",
            rate: "",
    ...
    updateSpecificAgency(id, payload)
      ...
      .catch((error) => {
        if (error.response.status === 401) {
          ...
          console.log(error.response.data);
          console.log(error.response.status);
          enqueueSnackbar("Unable to update agency.", {
            variant: "error",
          });
        }
        dispatch(unsetLoading());
  ...
  const deleteHandler = () => {
    ...
    deleteAgency(id)
      .then(() => {
        enqueueSnackbar("Agency delete successfully.", {
          variant: "success",
        });
        dispatch(unsetLoading());
        setRender("all");
      ...
      .catch((error) => {
        if (error.response.status === 401) {
          ...
          console.log(error.response.data);
          console.log(error.response.status);
          enqueueSnackbar("Unable to delete agency.", {
            variant: "error",
          });
        }
        dispatch(unsetLoading());
            ...
            <Card>
              ...
              <CardBody>
                ...
                <div className="d-flex mt-2 w-100 align-items-center justify-content-between">
                  ...
                  <RadioGroup
                    className="w-50"
                    value={travelPackageCommission.type}
                    onChange={(e) => {
                      const temp = {
                        type: e.target.value,
            ...
            <Card profile>
              ...
              <CardBody profile>
                ...
                <FormControl className="mt-4 w-100">
                  <InputLabel id="select-sub-plan-field" required>
                ...
                <FormControl className="my-4 w-100">
                  <InputLabel id="select-currency-field" required>
                    Select Currency{" "}
                  </InputLabel>{" "}
                  <Select
